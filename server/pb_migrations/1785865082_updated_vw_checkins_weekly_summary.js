...
migrate((app) => {
  ...
  unmarshal({
    "viewQuery": "SELECT\n  (ROW_NUMBER() OVER()) AS id,\n  date(c.checkInTime, '+8 hours', '-' || ((cast(strftime('%w', c.checkInTime, '+8 hours') as integer) + 6) % 7) || ' days') AS week_start,\n  c.branch,\n  c.method,\n  COUNT(*) AS checkin_count,\n  COUNT(DISTINCT c.member) AS unique_members\nFROM checkIns c\nGROUP BY date(c.checkInTime, '+8 hours', '-' || ((cast(strftime('%w', c.checkInTime, '+8 hours') as integer) + 6) % 7) || ' days'), c.branch, c.method\nORDER BY week_start DESC"
  }, collection)
  ...
  // remove field
  collection.fields.removeById("_clone_NyQP")

  // remove field
  collection.fields.removeById("_clone_1f9v")
