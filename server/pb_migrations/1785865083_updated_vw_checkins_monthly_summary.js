...
migrate((app) => {
  ...
  unmarshal({
    "viewQuery": "SELECT\n  (ROW_NUMBER() OVER()) AS id,\n  strftime('%Y-%m', c.checkInTime, '+8 hours') AS checkin_month,\n  c.branch,\n  c.method,\n  COUNT(*) AS checkin_count,\n  COUNT(DISTINCT c.member) AS unique_members\nFROM checkIns c\nGROUP BY strftime('%Y-%m', c.checkInTime, '+8 hours'), c.branch, c.method\nORDER BY checkin_month DESC"
  }, collection)
  ...
  // remove field
  collection.fields.removeById("_clone_0ep7")

  // remove field
  collection.fields.removeById("_clone_tktG")
